...
Safety verification establishes three things about a design: whether it detects physical faults, how much of their effects it detects, and what evidence supports that result.

In this example, eleven weeks before the flagship-A's tape-out, conventional evidence is complete: a verification plan with every row discharged, a coverage database at closure, a passing regression, and a bug curve that flattened a month ago. A bug curve is the plot of bug discoveries over time, and it is the main instrument of convergence. The functional-safety audit additionally requires the evidence requested by an assessor from the customer's safety organization.

The architecture specifies single-error-correcting, double-error-detecting protection for the tightly-coupled memory. Double-bit errors are reported to the safety island inside the fault-tolerant time interval. The fault-tolerant time interval is the budgeted span measured from the moment a fault occurs, and a mechanism must detect the fault and react within that span. The audit requires the faults injected into that path, those the mechanism missed, and the rationale for excluding faults from injection.
...
ISO 26262 dominates automotive functional safety and defines it as the absence of unreasonable risk from hazards caused by malfunctioning behavior of electrical and electronic systems [cit:D2]. That definition, and the rest of the vocabulary this chapter uses, belongs to Part 1 of the series: ISO 26262-1:2018, *Road vehicles — Functional safety — Part 1: Vocabulary*. DO-254 (RTCA, Inc., *Design Assurance Guidance for Airborne Electronic Hardware*, published by EUROCAE as ED-80) is the corresponding regime for airborne electronic hardware, and it appears in the same 2024 survey among the standards that teams comply with [cit:R2]. The concrete examples here are automotive; the obligations below apply equally to both regimes.

**Process obligations.** The standard prescribes how work is organized, not only what is produced: activities have prescribed inputs and outputs, reviews have prescribed independence, and changes have prescribed impact analysis. A controlled work product changes when a team deletes an obsolete test, upgrades a tool, or alters the scope of a coverage model. A coverage model is the multi-dimensional space of situations that a plan requires the stimulus to reach. Every such change requires a record of the decision maker and rationale.
...
- A **latent fault** cannot violate the safety goal on its own, and is neither detected by a mechanism nor perceived [cit:D32]. A fault counts as perceived when the driver notices its effect although no mechanism has raised an alarm. The fault remains latent until a second fault combines with it. For example, a dead lockstep comparator causes no problem until the two cores disagree and the mismatch goes undetected.
...
These classes feed the architectural metrics: the **single-point fault metric** (SPFM), the **latent fault metric** (LFM), and the probabilistic metric for random hardware failures (PMHF). Each carries a threshold per integrity level, tabulated in ISO 26262-5. The values reported for ASIL D are SPFM at or above 99 percent, LFM at or above 90 percent, and PMHF below 10 FIT; for ASIL C, SPFM at or above 97 percent and LFM at or above 80 percent; for ASIL B, SPFM at or above 90 percent and LFM at or above 60 percent, with PMHF below 100 FIT at both of those levels [cit:D12]. Those reported values come from that paper, not from the standard, and the standard is outside this book's corpus. FIT is the unit in which those PMHF thresholds are expressed: a failure rate, measured in failures per billion hours of operation. The standard also specifies the formulas behind those metrics: structural analysis and formal proof establish the fraction of safe faults, while fault injection measures the diagnostic coverage terms that determine whether the metric meets its target [cit:D32].
...
A workload runs twice: once on the unmodified design, the *good machine*, and once on a second copy that carries one hypothetical fault, the *faulty machine*. At designated strobe times, designated signals are compared; a difference between the two runs means the fault reached that signal [cit:D32]. A strobe time is an instant scheduled in advance for that comparison, and the list of those instants fixes what a campaign measures as much as the list of compared signals does.
...
- **Structural and formal arguments answer "under any stimulus".** The cone of influence of an observation point is the netlist region that can reach it. A fault outside the cone of influence of every observation point cannot reach an observation point under any workload whatsoever; this is a statically established property of the netlist, and it scales from block to system [cit:D12]. A proof engine is a formal tool that settles a question over every legal stimulus at once. When such an engine shows that a fault can never cause a difference at the observation point, that fault is likewise untestable by construction, not merely unstimulated [cit:D32]. These faults are safe and may be excluded.
...
**Collapsing** first sorts faults into equivalence classes, a *prime* fault represents each class, a *collapsed* fault is one producing the same observable behavior as its prime, and only primes are simulated [cit:D32]. **Structural pruning** follows, removing faults outside the cone of influence. Formal controllability and observability analysis then removes faults that cannot be activated or cannot propagate; controllability is the ability to steer a design into a given internal condition using only the interfaces that a testbench can drive. The faults left after these reductions are then simulated.

A design with faults at ports, flip-flops and wires starts from 3,606 faults; collapsing removes 973 and testability analysis another 144, leaving 2,489 to analyse. Structural cone-of-influence analysis marks 763 of those unobservable, leaving 1,726. Formal observability analysis then clears a further 248 (232 in one phase and 16 in a second), leaving 1,478 faults that reach the read-data port [cit:D12]. Testability analysis is the second reduction in the counts just given, and the published account names it as a step of its own. It runs before the cone-of-influence pruning rather than as one of the steps the figure above sets out.
...
> **Approach.** A single-bit fault deep in a twiddle-factor multiplier perturbs a bin's magnitude by a fraction of a least-significant bit. The detector's threshold sits far above that perturbation, so the detection list comes out identical: the fault changes the raw spectrum and changes nothing at the interface that the safety requirement is written against. The observation point belongs at the safety requirement's interface, the detection list handed to the fusion stage, rather than at the spectrum.
...
Timing adds one axis: whether detection meets the required interval. A safety mechanism must detect and react *within a budgeted time interval measured from the moment the fault occurs*, and must be running continually while the device operates; the fault-tolerant time interval and the fault reaction time interval are both things a fault injection campaign exists to establish [cit:D2]. Of the two, the fault-tolerant time interval is the one measured from the moment the fault occurs, and that interval covers detection and reaction together. The fault reaction time interval is the budgeted time that the mechanism has to act once it has detected. The campaign distinguishes detection in time from late detection, which counts as undetected despite the alarm log.
...
**Silence during healthy operation.** Absence of spurious alarms is also a verification obligation. False positives are a reliability defect in their own right. They can be worse than the fault a mechanism guards against, if they fire on healthy parts in the field more often than that fault would occur. A spurious trip takes the system directly to a safe state rather than degrading gracefully: in a vehicle, a warning lamp, limp mode or dealership visit. Verification that asks only the first question can miss spurious trips that cause returns from otherwise healthy parts.

The two checks differ in scope. Injecting a fault through a script demonstrates whether the mechanism fires. Proving it stays silent means running the design's *entire legal operating space* with the alarm never asserting, and that obligation has no natural stopping point. It is the shape of any must-not-happen requirement (Chapter 5) and is discharged the same way: an always-on assertion in every regression run, plus a demonstration that the assertion can actually fail.
...
What makes a lockstep pair effective is that the two cores fail *independently*. On the flagship-A the pair is delayed: the shadow core runs some cycles behind, so a disturbance hitting both at the same instant hits them at different points in their execution and produces a mismatch instead of two identical wrong answers. That delay, and the separation behind it, is the mechanism. Verification must establish three things. The first is whether the delay pipeline preserves comparison across every stall, flush and interrupt. The second is whether the two cores share any resource whose single failure would corrupt both identically, such as a clock buffer, a power rail, a memory port or a reset synchronizer. A reset synchronizer is the structure that asserts reset asynchronously and releases the reset on a clock edge, so that no flop meets the release inside its recovery window. The third is whether something else catches the comparator's own failure. That last question is the latent-fault question again, and its usual answer is a periodic self-test that provokes a mismatch on purpose and confirms the alarm.
...
Tool qualification is the response (ISO 26262-8:2018, *Road vehicles — Functional safety — Part 8: Supporting processes*). Tool qualification asks two questions. The first is how much damage the tool could do by malfunctioning, either by introducing an error into a safety-related work product or by failing to detect one already there. The second is how likely something else in the flow is to catch that malfunction. Greater potential damage and less independent checking require more supporting evidence, supplied through a vendor qualification package, a demonstrated history of use, or the project's own confirmation testing, a suite that the project runs on the tool itself, to check its output on the use cases that the evidence relies on.
...
This is the industrial form of the known-bad variant from Chapter 8: instead of keeping one deliberately broken copy of the design in the repository, the flow builds a systematic population of small mutations, runs each against the environment and classifies each as detected or not. Undetected mutations identify places where a real bug of the same shape would also have gone unnoticed, pointing to a missing checker or an unsampled signal rather than to a missing test.
...
Three plan rows serve three obligations under Chapter 5's one-row-one-metric rule, which applies at least as strictly to safety: each plan row must be discharged separately for assessment. The cover directive from the artifact above needs no row of its own: row `FA-SM04a`'s campaign hits it the first time an injected double-bit error is detected, and a campaign in which it is never hit has already failed that row. Row `FA-SM04c` addresses the plateau by requiring an argument for faults left unobserved; it does not promise that every fault is safe, but prohibits silently assuming safety.
...
Resentment toward both documentation and fault campaigns can undermine safety work, while a team that treats all documentation as engineering risks burnout. When the schedule slips, the plan should distinguish engineering work that needs protection from documentation that needs automation. Generating traceability artifacts from the plan and run database avoids maintaining them by hand. The funding plan should identify this as a tooling project and avoid postponing it until the second program.
...
**Workload selection requires agreement.** Diagnostic coverage is measured under a workload. In this example, leaving half the design idle leaves half its faults unobserved, on two assumptions: that the idle portion contains the corresponding share of fault sites, and that the workload does not excite those faults. The workload must activate the mechanisms yet remain short enough for thousands of faulty-machine runs within the compute budget; its written engineering rationale is one of the first items an assessor reads.
