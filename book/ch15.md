...
§15.1 explains why the pre-packaged formal *app* succeeded where general property checking stalled: the tool supplies properties, so a team supplies only the design and a configuration. §15.5 examines the one app that extracts its properties from the design itself.
...
§15.6 states what logic and sequential equivalence checking each prove and assume, and why logic equivalence checking won full industrial acceptance while property checking was still developing. §15.7 explains why arithmetic datapaths need their own flow.
...
A safety app checked an ECC block at NXP Semiconductors using SECDED (single-error correction, double-error detection) around an 8K memory. When the safety app was run on the whole block, the fault campaign did not converge in even 12 hours, so the memory was blackboxed and its input ports promoted to observation points, the places where a fault becomes visible to the outside world. The authors characterize this abstraction explicitly because it can produce false negatives on the safe classification but not false positives. A prime fault is the representative of a class of faults with the same observable behavior. With that abstraction, the campaign ran in stages: it cut 3,606 injected faults to 2,489 prime faults; it proved 763 of those structurally unobservable; it then proved a further 232 formally unobservable, and after that another 16; and it ended by identifying 512 faults that corrupt the stored data without ever raising the block's error signals [cit:D12]. The app supplied the fault model and machinery for pruning and classification, but a human supplied the abstraction needed for termination.
...
A positive connectivity table leaves the equally important requirement for forbidden connections unspecified. The reference SoC's debug fabric supplies negative rows: the JTAG-reachable abstract-access path may reach memory and the core's register file, but must not reach the event unit's retention registers. The abstract-access path is the debug fabric's own route for reading and writing memory and registers, and retention registers are the ones that keep their contents while the block around them is powered down. A no-connection check is cheaper because it asks structurally whether any path exists in the cone, without invoking a proof engine.
...
> **Why.** This structural security check catches a debug hook, a test observation point or a "temporary" mux added during bring-up, the phase in which environment and DUT first exchange checked transactions. A no-connection result is *structural*: no path exists. It does not say the key never influences an observable value along a path that does exist, which is an information-flow question and belongs to Chapter 23. Structural non-adjacency alone does not prove non-leakage.

Checking every path of a multi-port interconnect individually has poor runtime and, by symmetry, minimal verification value. The crossbar's five manager ports against its four subordinate ports make twenty combinations, and far more once each combination is qualified by burst type and identifier. With *symbolic* port indices, an arbitrary manager M and subordinate S are constrained only to remain stable, and the single path from M to S is checked. Because the proof must hold for every value the arbitrary indices could take, one property covers all of them. Symbolic checking is not always faster: 162 seconds against 65 for all exhaustive per-path proofs together [cit:D13], so runtime needs measurement. It keeps the connectivity argument constant-sized as the interconnect grows.

## 15.3 Register checks: the map against its description

Chapter 10 built a register model and used it to drive and predict accesses. That model was generated from the subject of this section: a register description language (Accellera SystemRDL 2.0 Register Description Language, Version 2.0, 2018), one file from which every view is generated [cit:S2]. The views so generated include RTL decode and field logic, the verification model, firmware header and documentation; maintaining them separately allows drift.

The register app proves that RTL implements the description. From that description it generates and proves reset, access-policy and functional checks for every field [cit:D13], plus the implementation-level properties the description implies but does not state. For each register the app proves this much: the reset value is what the description says, for every reset; the decode maps exactly one address to it; a read returns what the policy says; and a write does what the policy says *for every value written*, not for the values a test happened to pick. It also proves that reserved fields ignore writes and that unimplemented addresses behave as declared.

A conventional register test walks the map front-door, register by register and pattern by pattern, through write-read-verify sweeps, reset checks and bit-bash, driving real bus cycles rather than reaching the registers by hierarchical path. In a bit-bash the test writes and reads back each bit of a register in turn. Its cost scales linearly with the map, so a large map's register test takes hours. The app proves the same statements over all values and orderings in one job, at RTL rather than through the bus, so it converges even for registers a bus-level test reaches only through a long configuration sequence.
...
Chapter 6's coverage closure distinguishes stimulus gaps from unreachable targets. Some holes are genuine stimulus gaps requiring closure. Other holes are targets no legal execution reaches, and they warrant an exclusion in Chapter 7's written form: the hole, reason, risk argument, owner and expiry. For an unreachability exclusion, the reason is the engineer's claim that a state is unreachable.
...
Reachability depends on the constraints, and a wrong constraint makes reachable behavior look impossible. An input constraint in a tool setup script (`enable_cnt == 0`) made a counter's increment line uncoverable; the line was reported as unreachable, and under that constraint it really was unreachable, though the constraint was only an assumption, one that no engineer had reviewed and that did not reflect the design's real environment [cit:D11]. An unreachability verdict inherits every assumption in the environment, including ones written in a scripting language by someone who has left the project. The constraint set should be evidence, reviewed as seriously as the waiver it replaces.

The second limitation is structural: the app works on the *design* (RTL lines, branches, FSM states, cover properties bound into design scopes), so a functional coverage model computed by the testbench is invisible to it. The canonical DMA coverage model is `cg_2d_4kb` from Chapter 5. Its cross `x_worst` pairs every value of each axis with every value of the others, and has **72** combinations: 3 stride signs × 4 boundary relations × 3 burst lengths × 2 states of the other channel. A stride is the signed address step from the start of one row to the start of the next. The other channel is the DMA's second transfer engine, independent of the first, with a state of its own and a share of the AXI backend. **Six** of those combinations, the whole `straddle × max` column, are excluded: three stride signs by two other-channel states. They are excluded by *policy*, which Chapter 9 derives, rather than by a design property, leaving **66** cells to close.

The app cannot see that cross because `txn.bnd_rel` lives in the monitor. A machine verdict requires a design-level restatement in terms of the burst splitter, the DMA stage that splits each transfer into bursts that the AXI address rules allow. The design's own code calls the burst splitter by an older name, the midend legalizer, and the identifiers below keep that name:
...
The engine answers in seconds: the cover is **reachable**, with a trace showing how. Nothing in the design bounds a 2D row at one maximum burst; a 2D row is the contiguous run of bytes that makes up one line of a two-dimensional transfer. The bound that empties the column lives in the constrained-random generator (legal but unlikely stimulus, generated automatically under constraints), and a design-level cover does not inherit that bound.

The two questions are related but distinct. The design-level cover asks about the burst splitter's own split flag and burst length; the six-cell figure is an argument about the monitor's per-row `bnd_rel` axis and about how that axis pairs with a post-split length sample. The exclusion remains valid as a stimulus claim, not a design claim: true while the generator is unchanged, false when it widens.

The app therefore cannot discharge this exclusion; the limitation is structural. To return *unreachable* the tool would have to be told about the generator's budget, and that budget is not a property of the design: Chapter 9 labels its bounds engineering choices about solve time rather than statements about the DUT. Assuming that budget would make the verdict inherit it, recreating the first limitation. A disposition is the decision recorded on an open item, and the one recorded here remains a waiver whose expiry is an edit to the constraint file; that edit must count as verification evidence.
...
Every other app finds bugs by comparing the design with external intent from a wiring table, register description or coverage model. Here, intent and implementation coincide by construction. A golden reference is an independent statement of intended behavior, trusted as the standard that the design is checked against. The limitation is structural rather than a matter of tool maturity: automatic property generation divides into two families, dynamic mining from simulation traces and static analysis of a specification; the mining family generates and evaluates its properties on the same RTL, and it has no golden reference, so flaws in the RTL yield flawed properties, and mining has no independent way to detect them [cit:P11].

A mined property derives from the behavior of its source RTL, which is why it proves on that RTL, and why it would prove in the same way if it had been mined from RTL with the opposite behavior. Its bug-finding power against today's RTL is zero, and no amount of proof depth changes that. Its regression value is to make current design behavior an obligation against which subsequent changes are checked.
...
An equivalence checker is also a large software system: one study randomly generated SystemC programs built by construction to be equivalent to trivial RTL, tested them against three major commercial C-to-RTL checkers, and uncovered sixteen distinct defects [cit:D46]. A C-to-RTL checker is a different class of tool from the RTL-to-netlist checkers above: it compares a model written in C or SystemC against the RTL. The paper withholds tool names under license, so the result concerns the class rather than a ranking.

**Sequential equivalence checking (SEC)** removes the matched-state-point assumption at greater computational cost. Two machines are equivalent when every input sequence produces the same output sequence; deciding that means computing the reachable states of a product machine that runs both designs in lock-step on the same inputs and reports whether their outputs agree [cit:P20]. Computing those reachable states is a model-checking problem, with model checking's costs, so SEC is aimed at bounded, well-chosen transformations rather than whole designs; it covers the cases that LEC cannot handle, rearchitected finite state machines and restructured data pipelines, and on those cases modern equivalence checkers handle sequential differences between two RTL models [cit:B2]. The following two cases concern clock gating and retiming.
...
A design's state space is the set of all configurations its state elements can take. For control logic, simulation's coverage argument is a reasonable approximation: the interesting behavior lives in state sequences, the state space has structure, and a suitable coverage model samples it. For arithmetic, meaningful sampling by a coverage model is infeasible. The video codec's inverse transform illustrates the input space. An 8×8 residual block is 64 coefficients; at 16 bits each, the input space of a *single* invocation is 2<sup>1024</sup> values. No coverage model meaningfully samples that space, even with a billion random blocks. The failure mode is scattered across the input space: a rounding term applied at the wrong stage, a saturation clipping one bit late, an intermediate width one bit short; each is wrong for a sparse, structured subset of operands that random stimulus meets with probability near zero. The codec's compression standard specifies the inverse transform as exact integer arithmetic precisely so encoder and decoder cannot drift, which makes a one-bit mismatch in one coefficient a conformance failure rather than a quality issue.

An arithmetic block's specification is a mathematical relation between operands and result, which can be written directly. The formal verification of floating-point hardware after Chapter 1's Pentium division episode established this pattern: a multiplier's top-level specification is a formalization of the arithmetic standard, and it states a relation between the input operands and the result in terms of arithmetic on integers. The significand is the digit string of a floating-point number, the part that the exponent scales; the specification reads the significand and exponent vectors as integer values rather than as bit vectors. Proofs at that level are decomposed: over 120 properties covering all arithmetic operations of one industrial floating-point unit, each a property about a subunit (a loop invariant where the algorithm is iterative), with the irrelevant parts extracted away per property [cit:P20]. A loop invariant is a statement that holds on every iteration of the loop. Two requirements follow: word-level reasoning and decomposition, because a monolithic claim does not converge.

The industrial flow that grew from this compares an algorithmic reference (typically C or C++, bit-exact, often the code the architects used to develop the algorithm) against the RTL, with the operand and result correspondence declared by the user. In that description of the flow, the clause about the user declaring the operand and result correspondence is practitioner judgment: this book's corpus does not document it, and no source is cited for that clause.
...
Two published cases put that flow at opposite ends of its cost range. A reference model is an independent executable of the specification, taken as golden and run on the same operands as the design. Arm and Mentor Graphics proved floating-point operations of Arm CPU designs sequentially equivalent to C reference models, and published the effort per operation. Proof development took two days for FADD, and two months for each of FMUL, FDIV and FSQRT; the proof runtimes ranged from forty minutes to twenty hours. The figure that motivated the work was one hundred and fifty days of CPU time to simulate one operation with two half-precision operands exhaustively, and what the proofs bought carries the same scope: exhaustive simulation was eliminated for half-precision operations, not generally [cit:D37].

Intel's graphics organization reports this comparison used for early bug finding. The comparison started before the design and the C++ model were finished, as an exercise aimed at early bugs rather than at the complete coverage a sign-off claim requires, and it produced eighteen bugs in under a week, in the C++ model and the RTL together, with the split between them not stated [cit:D38]. A plan must distinguish the counterexample budget from the evidence needed for a proof.

> **Scenario.** The radar front-end's constant-false-alarm-rate (CFAR) detector accumulates FFT magnitudes in a fixed-point format the algorithm team chose from a floating-point precision study. Datapath verification proves the RTL bit-exact against the team's fixed-point C model, on every operand.
...
A plan row is the line of the verification plan that carries one feature: its attributes, a pass/fail criterion, a method and one closure metric, the evidence that decides when the feature counts as covered. The plan gains two of them in Chapter 5's DMA format ({ref: soc_connectivity_plan_rows}):
