...
FPGA prototyping supports hardware/software integration and lab debug; this chapter examines how debug methods can reduce the nine days spent locating the flagship SoC's DMA bug. In this book **validation** checks a design on real hardware rather than a model: post-silicon on fabricated parts, *or in the lab on a prototype* (glossary; Chapter 20 states it in full). An FPGA prototype is therefore pre-silicon in *time*, because it runs before any part is fabricated, and validation in *kind*, because by that definition the hardware carrying the design is real. It is the first platform on which real software runs against real hardware, and the first on which bug cost is determined by lab debug ergonomics rather than by simulation's ergonomics.
...
The verification team cannot reproduce it: their DMA and coherence tests pass, while the failing workload spans hundreds of millions of cycles, beyond their simulator's reach this quarter. The software team's driver is the same driver that has run for six months against a functional model without a single failure. Available evidence: one console, a few LEDs, and a trace buffer configured for a different question three bitstreams ago. Two teams spend nine days deciding whose bug it is. The flagship's DMA must split every transfer into bursts that the AXI address rules allow, none of them crossing the 4 KB boundary, the address line that AXI forbids a burst to cross so that no burst spans two subordinates. The DMA stage that performs that split is called the burst splitter, and on the flagship it mis-splits a 2D transfer descriptor at that boundary. A transfer descriptor is the record software writes to program one DMA transfer. The flagship therefore inherits the reference SoC's canonical bug, the book's recurring defect. Here the same defect sits one layer deeper: coherence hides the corruption until writeback, because the second half of the split writes a line the host cluster holds Modified. A cache holds a line Modified when it has written to that line and has not yet returned it to memory. The bug itself was straightforward, but locating it took nine days because neither team could see it on the prototype.
...
The canonical wake-cause bug occurs at that boundary: a 3-bit code crosses from the always-on island to the system domain through per-bit synchronizers, briefly exposing reserved code `3'b110` during a legal `3'b010 → 3'b100` transition. Reproduction depends on how many fast-domain sampling edges fall within the slow domain's transition window, and the clock change alters that number. A prototype that never reproduces a crossing bug and one that reproduces it constantly can be the same design at two clock settings. Structural CDC analysis found this bug in seconds (Chapter 13); the prototype is not the instrument for it, neither when it never reproduces the crossing nor when it reproduces it constantly.
...
The flagship's LPDDR4 controller and PHY cannot be prototyped, although their training state machine is among the riskiest things on the chip: the PHY is analog and its physical timing determines its behavior, so the board replaces them with the platform's own memory controller. The prototype can run the OS but establishes nothing about memory training, a bring-up risk that must be discharged at the analog and mixed-signal boundary (Chapter 21) and in the post-tape-out lab (Chapter 20). The plan must record an explicit prototype-scope exclusion, as Chapter 5 demands for every gap.

### Prototype and emulator roles

Both platforms run RTL on hardware, and both reach megahertz-scale effective speeds on large SoCs, with the achievable frequency depending on design size, partitioning, instrumentation and the selected debug configuration [cit:P14]. An emulator provides capacity and controlled observability through triggers, selective trace, save and restore, with compile and debug infrastructure sized to the design (Chapter 17). A prototype provides speed and realism, with observability limited to instrumentation inserted before the build. *Emulation* synthesizes and partitions the design, then compiles it onto a hardware execution fabric where it runs under stimulus a host supplies; what that fabric runs is a model of the design rather than a fabricated part, so the work stays pre-silicon. The 2024 study reports sharply rising emulation adoption above a billion gates [cit:R2], a capacity threshold where prototype partitioning becomes a substantial project.

An emulator serves as a fast simulator with reduced visibility; a prototype approximates early silicon with more visibility than silicon; the two sit one on each side of the verification/validation line. Work limited by simulation speed belongs on an emulator, while lab work awaiting a chip belongs on the prototype.
...
> **Scenario.** The video codec's encoder passes its block-level environment: a bit-exact golden model checks every macroblock against the reference encoder, and the coverage model closes on prediction modes, quantiser steps and buffer-full conditions. The golden model is the one trusted because it is the specification itself; the coverage model is the multi-dimensional region that its closure conditions and their values mark out. The untested sequence is a thousand consecutive frames of real content, with rate control adapting across scene changes while DDR traffic shifts. In simulation that is weeks. **Approach.** The encoder runs on the prototype, receives a real clip from a file and writes the bitstream to storage; the entire output is then diffed offline against the bitstream the reference encoder produces, in one comparison on a workstation after the run. **Why.** It satisfies all three admission conditions. The workload needs the cycles; its offline bit-exact oracle needs no visibility; and if frame 738 differs, its number and the encoder state that produced it suffice for a simulation testcase using the same input.

A bug escape is a design flaw that survives verification into a later stage, found on the wrong side of the boundary that was meant to catch it. In the FPGA-target market segment, designs ship in fabric, and the lab, hours away, routinely serves as the primary validation vehicle; many teams minimize pre-lab verification to reach that lab quickly, and the strategy worked for simpler devices but is increasingly impractical for modern SoC-class ones; in the 2024 Wilson Research Group FPGA study, only 13 percent of projects in that segment reported zero non-trivial bug escapes into production [cit:R1]. Using the prototype as the verification instrument gains only speed: it gives up simulation's control, visibility and reproducibility for an environment that has no control, no visibility and only weak reproducibility.
...
A virtual platform establishes four things at once: that the initialization sequence sets the registers it means to set, in an order the hardware accepts; that firmware's view of the memory map agrees with the architecture's; that the handlers connect to their intended sources; and that device tree, linker script and boot ROM hand-off agree. The device tree is the data structure that describes the hardware to the operating system at boot, and the linker script is the file that decides where each section of a compiled program sits in memory. On the flagship that is four cores' worth of OS bring-up, a driver stack and a filesystem, debugged with full state inspection, none of it waiting for RTL.

The platform's limits follow directly from preserving only the basic functionality of the hardware/software interfaces. It has no arbitration, because there is nothing to arbitrate. It has no back-pressure, because a model that returns immediately never stalls. It has no latency, so two operations that the design will overlap are sequential in the model. It has no coherence, because a single flat memory has nothing to be coherent with. Bugs involving *two things happening at once* are absent by construction, so the platform reports success without testing them.

Two omissions produce firmware that passes on the model and fails on hardware:

- **Untested timing assumptions.** A poll loop that reads a status register until a bit sets terminates on the first read against a model that completes instantly, leaving the lack of a timeout and memory barrier unnoticed. A `udelay(10)` that was chosen because it felt safe is never tested against a device that takes longer.
...
A second weakness appears when the virtual platform becomes the specification by default. When model and RTL disagree, neither is golden: both were written by people reading the same document, so a disagreement is as likely to be two misreadings as one bug. Where they agree wrongly, a common-mode error remains, an error the two artifacts share, and comparing them cannot bring it to light (Chapter 3). Chapter 15's structural fix generates model register behavior, decode logic and the firmware header from one register description, reducing disagreements to behavior rather than layout.
...
Arm IHI 0050, the *AMBA CHI Architecture Specification*, Issue H (2025), gives a coherent interconnect a seven-state cache model; the specification then states the cache state transitions twice over, as a table per transaction type: once at a requester and once at a snooped cache [cit:S26]. A requester is the cache or device that asks the interconnect for a line; a snooped cache is one that the interconnect then questions about a line it may hold. Every one of those rules applies on the prototype but has no counterpart in the driver's development platform, so missing cache maintenance reaches the board looking like a hardware bug.
...
An untimed model offers no timing to trust, whereas an approximately timed model supplies plausible timing numbers that can be quoted and that still differ from the design's. Tuning a firmware queue depth, a batch size or an interrupt coalescing threshold against this timing produces a number that reflects the model rather than the design; review slides should identify those numbers as model estimates. An interrupt coalescing threshold is the number of events that a device waits for before raising one interrupt. An approximately timed model answers *architecture* questions with ratios or trends; it cannot answer *verification* questions requiring a verdict.
...
The industrial form partitions the system across two execution domains: a virtual platform on the host, running high-level or fast instruction-set models, and an engine that executes selected RTL blocks at cycle accuracy; transaction-level co-modeling channels connect those two domains, so software workloads execute quickly while the hardware behavior under verification stays at RTL fidelity [cit:P14]. The RTL side may be an emulator (Chapter 17) or, for smaller blocks and shorter workloads, an ordinary simulator; the architecture of the split and its consequences are the same either way, because the single channel that {ref: hybrid_partition} shows between the two sides carries transactions, never clock edges.
...
{figure: hybrid_partition} A hybrid co-simulation partition. On the virtual side, the operating system and the driver under development execute untimed on the host, which runs an instruction-accurate model of the application core alongside console, storage and network models. On the RTL side, the block under verification and its bus interface run cycle-accurate. Everything between the two sides crosses one untimed transaction-level channel: register reads and writes go inward, interrupt and status events go outward. That hand-off is what deletes every bug that lives in the timing relationship between the two sides.

### Three motivations and partition criteria

Published motivations also define boundary-placement criteria. First, hybrid co-modeling *reduces the RTL footprint* by keeping well-modeled components virtual; second, it exploits *model-maturity asymmetry* through mature virtual models, typically CPUs and standard peripherals, while retaining RTL for subtle or insufficiently modeled behavior; and third, it *balances throughput against fidelity*, accelerating end-to-end workloads while preserving cycle accuracy where timing, ordering and microarchitectural behavior matter [cit:P14].

The boundary belongs where three things are true at once:

1. **The boundary already has architectural transaction semantics:** a memory-mapped register interface, bus port or mailbox. Its contract must specify operations because the channel can only carry operations. A boundary across microarchitecture, such as a pipeline, credit counter or handshake, has no transaction to send. *Transaction-level modeling*, or TLM, means moving whole transactions between components over standard interfaces rather than signals. The TLM standard is built on SystemC and forms part of IEEE 1666, which defines SystemC and supplies an interoperability layer for models written above the level of signals. A published analysis of TLM-2.0 reports that its interoperability guarantee reaches only memory-mapped bus protocols; for serial links and interrupt lines, every designer defines a payload of their own, leaving such boundaries dependent on channel definitions local to the project [cit:D43].
...
The channel between the domains is untimed. A transaction crosses as a hand-off rather than a waveform: the virtual core issues a write and the RTL side executes it in its own time. Between the moment the core issued the write and the moment the block saw it there is a relation, and that relation is an artifact of the co-modeling implementation, not a property of any system that will be built.
...
**Initialization ordering.** Hardware/software interaction is rarely a single access; it is a sequence that must follow rules and respect environmental constraints, and those access constraints and preconditions are frequently *not documented* despite being required by the device hardware or by the bus systems between that hardware and the core [cit:T1]. A testbench writer reads the register map and writes accesses to it; a driver writer reads the same map and produces the sequence the software actually needs: power-up order, clock enables, a PLL lock poll, a reset deassertion, and a queue base address written before the queue is enabled and not after. Silicon executes that sequence throughout its life; verification should check whether it is the only sequence tried.
...
**Error-recovery paths.** Real drivers contain recovery logic: timeouts, retries with shorter bursts, channel resets and reinitialization from scratch. A directed test is the kind of stimulus whose content and order are written out by hand, doing the same thing on every run. Directed tests cannot derive the driver's recovery sequence from the register map; an engineer inventing one produces a different sequence. Running the driver under conditions that activate recovery exercises its actual recovery sequence.
...
At AMD, test intent written in the Portable Test and Stimulus Standard was compiled into UEFI and bare-metal tests, and those tests ran unchanged on an emulated AMD APU and then on post-silicon parts, reaching the hardware through an in-house runtime layer called uDREX, which abstracts the BIOS and starts the test on every enabled thread; on the emulator, the test and uDREX were backdoor-loaded into DRAM; and in the lab, the team used the same descriptions to compose fresh power-management firmware tuning scenarios instead of the fixed set a shipping driver happens to exercise [cit:D29]. Backdoor-loaded means written straight into memory by the platform, without the bus transfers that a processor would issue to put it there. The slide deck that reports the AMD work gives no bug count, speedup or effort saving; what the deck does record is test generation time that the authors warn has the potential to increase exponentially. A lab engineer or architect can compose a system-level sequence in the afternoon it is needed, supplying variation the driver alone structurally cannot. Chapter 9 gives the language itself its own treatment.
...
The reference SoC's DMA carries this book's canonical protocol property, and the flagship's DMA inherits that property unchanged, because the flagship's burst splitter is subject to the same discipline at the 4 KB boundary (Arm IHI 0022H.c §A3.4.1) [cit:S18]:
...
A concurrent assertion is a clocked, temporal assertion, evaluated at clock ticks on the values sampled at those ticks and checked on every run that executes it. Concurrent assertions are a simulation and proof construct; there is no assertion engine in the fabric, and where a toolchain offers synthesizable assertion support, what it can report is bounded by the same visibility limits as everything else. The check must become logic reporting its verdict through a driver-readable register.
...
When deterministic replay is unavailable, repeating the run is what checks whether its result is stable. The 2026 flow of §18.1 encounters this limit: its OS boot is a single indivisible test that cannot be distributed across the boards the rest of the suite parallelizes over, so it is executed eight times instead to establish that the result is stable [cit:P25]. This applies Chapter 12's repeat-*N* stability check where determinism cannot be assumed, to distinguish intermittent failures from mistaken observations.

**2. One timeline.** Firmware writes a log; in-fabric monitors latch into registers; the trace buffer captures bus activity. If those three record time in three different ways, an engineer establishing whether the driver's timeout preceded or followed the DMA's error must guess. A common timestamp source turns three records into one ordered timeline: a free-running counter readable by software and sampled by the monitors at a resolution finer than the events to be ordered. The always-on domain's 32 kHz reference is not that counter. Adding the counter is cheap before the build and impossible within an already built bitstream.
...
> **Scenario.** The radar front-end's firmware computes per-channel calibration coefficients at start-up and writes them into the FFT chain's correction registers. On the prototype, detections are consistently off: targets appear at roughly twice the expected amplitude, and the threshold logic of the constant-false-alarm-rate (CFAR) detector behaves accordingly. The DSP team demonstrates the datapath is bit-exact for every input it was given, against its reference model, the independent executable of the specification that is run on the same stimulus and compared with the design. The firmware team demonstrates the coefficients are correct, checked against the calibration procedure. **Approach.** Triage should examine the register description rather than dispute the two implementations. The correction register is 16 bits wide, and the document says only "signed coefficient". The RTL uses Q2.14, with two integer bits and fourteen fractional bits, while firmware computes Q1.15. Both are self-consistent, both match the document, and the values differ by a factor of two. **Why.** This is the third triage outcome. Neither software nor hardware is wrong: the field interpretation was undocumented, and the two teams each supplied the missing half from their own conventions. The remedy requires a register-description entry, not a patch; it generates decode logic, model and firmware header (Chapter 15), alongside a plan row asserting the interpretation to prevent recurrence with the next 16-bit "signed coefficient".
