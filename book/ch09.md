...
Stimulus generation ranges from directed tests to constrained-random and portable scenarios; the DMA example starts with a testbench that is three weeks old. Its driver and its scoreboard, the structure that holds what each transfer should produce and checks every output against it, support eleven directed tests, all passing. Test 1 programs a 64-byte 1D
copy and compares the two buffers. Test 7 programs a 2D transfer with a positive row stride, the signed address step from the start of one row to the start of the next. Test 11 programs a destination eight bytes below a 4 KB boundary, the address line that AXI forbids a burst to cross: every burst stays inside one 4 KB address region, so it never spans two subordinates. The burst splitter is the DMA stage that splits each transfer into bursts that the AXI address rules allow; it sits between the register frontend and the AXI backend, and this test checks the splitter's split against the 4 KB rule. Selecting test 12 exposes the limits of directed enumeration.

Candidates include negative strides, both channels at once, and `NUM_REPS` at its maximum.
Another is a 2D row, a single line of a two-dimensional transfer, exactly one maximum burst long. It starts exactly one maximum burst below a 4 KB boundary, on DMA channel 0, the first of the DMA's two independent transfer engines, while channel 1 drains. Each is plausible and takes half a day to write and debug; the list has several thousand candidates rather than eleven more. Hand-written tests are slow and find only anticipated bugs.

Constrained-random generation replaces test enumeration with *rules*: a model of legal transfer descriptors, the records that program one DMA transfer each, lets an engine search for the values that satisfy every active constraint and generate descriptors by the million, while coverage measures the results. That engine is the *constraint solver*. This shift from tests to rules introduces a new class of weakness: stimulus that is abundant and legal, and that still reaches none of the situations that matter.
...
Individual testbenches verify individual features: the stimulus is manually crafted to exercise one feature, and the response is checked against the symptoms that would appear if that feature were broken [cit:B2]. Nothing is chosen
...
debug when everything is broken at once. When the DMA engine's first descriptor fails, debugging must establish whether the fault is in the register decode, the burst splitter, the backend, the scoreboard or the clock. A twelve-line directed test answers
...
authority, and randomizing it violates that enumeration. An HBM controller's initialization and PHY-training sequence, the opening handshake of a high-bandwidth memory interface, is a strictly ordered protocol with mandated wait states between steps: there is exactly one legal opening move, and the test that checks that opening sequence is directed. Any published compliance suite likewise requires its specified tests rather than a random sample from a separately defined space.
...
**Where it stops.** The directed approach can be managed to completion when the total number of test cases is in the low hundreds; a project with over a thousand identified test cases would need more than a year this way, and beyond that some form of testbench automation becomes necessary to complete the task within an acceptable time frame [cit:B2]. That necessity is a claim about schedule rather than about feasibility. The reference SoC's DMA alone generated several thousand
...
The second is **decay**: a directed test can pass forever after it stops testing its intended feature. Enlarging a FIFO or memory leaves tests of its full operation passing while they exercise only part of it. The defense is a redundant path: a coverage model, the space of situations a run is measured against, states what the test is supposed to accomplish and requires it to fill 100% of those points, since deterministic stimulus should fill exactly what it targets [cit:B2]. A directed test that drops to 60% of its own goal has become stale.
...
remainder [cit:P10]. One methodology recommends a directed test when a scenario is too complex to describe or too unlikely to happen randomly; a run need not be 100% directed, since a directed sequence can be injected randomly within a stream of random ones, and injecting a directed sequence in that way may expose problems a purely directed run would not [cit:B1].

## 9.2 The constrained-random turn

Constrained-random generation addressed bugs that appear only in convoluted scenarios, where states across several state machines align only through one particular transaction sequence; once a team had written as many directed tests as it could, random simulation was the only reasonable response left [cit:B6].
...
Dedicated hardware verification languages developed this model before general standardization, and the `e` language records it. As a commercial product, `e` came from Verisity Ltd., and what put the language on projects was Specman, Verisity's flagship product; the language's designers noted that most of Specman was itself written in `e` [cit:P18]. In it, a data item is a struct whose fields carry declarative
...
generators that can be constrained easily from the outside [cit:B2]. Two commitments follow, and both apply from day one of testbench development: functional coverage, and a generator built to be constrained from the outside.

First, **functional coverage is required**, because only the coverage model measures what actually ran; the generator describes what could happen. Without functional coverage, a random environment is merely running directed test cases whose stimulus is filled in at random [cit:B2].
...
constraints layered on top, which steer the run toward the scenarios currently targeted. The figure shows the two families as three nested regions [cit:B6] ({ref: constraint_space_regions}):
...
The DMA's 2D descriptor generator labels each constraint by family. The DMA's 64-bit data path and its maximum burst of 256 beats, a beat being one data transfer inside a burst, give 2048 bytes per maximum burst, exactly half of one 4 KB address region.
...
Four details distinguish legality from policy. The `c_addressable` comment distinguishes programming from execution. An address outside the memory map is legal to *program*, because the register field accepts it. What the DMA then does with that address is a decode error reported per DMA channel, and that response is a feature the stimulus must exercise. Restricting stimulus to the uses that the design is intended to see is the error Chapter 2 warns against, and such a restriction can be made permanent in a `constraint` block. Second, `c_solver_budget`'s bounds are engineering choices
...
Random values shall be selected to give a uniform distribution over legal value *combinations*, with every legal combination equally probable, so that randomization explores the whole design space [cit:S8].
...
> **Scenario.** The radar front-end's FFT pipeline takes 16-bit samples in Q1.15, a signed fixed-point format whose values carry a sign bit ahead of the binary point and a fractional part behind it.
...
`dist` assigns weights to values and ranges: absent any other constraints, the probability that the expression matches an item is proportional to that item's weight. Of the two operators, `:=` applies the weight to each element of a range, while `:/` divides one weight across the whole range. The weights are ratios and need not be normalized to 100. Nonzero weights do not change the solution space, and therefore they cannot cause the constraint solver to fail. Other constraints on the same expression renormalize the remaining choices. Values *absent* from the list or with *total* weight zero are forbidden [cit:S8]. A `dist` whose ranges do not cover everything legal is silently an
over-constraint.

`solve... before` changes probabilities by ordering choices. With `s -> d == 0` over a 1-bit `s` and a 32-bit `d`, the legal combinations number 1 + 2³²: `s` true pins `d` to zero and admits a single combination, while `s` false leaves `d` free. `s` is true in exactly one of those combinations, and two of them leave `d` at zero: that combination, and the one in which `s` is false and `d` is drawn as zero. Those two give `d == 0` a probability of 2/(1 + 2³²), effectively never. Adding
`solve s before d` leaves the legal set identical but makes `d == 0` occur with
probability slightly over 50%. Variable ordering forces selected corner cases to occur more often and, like `dist`, cannot cause the constraint solver to fail because it does not change the solution space [cit:S8]. Variable ordering addresses the pattern in configuration descriptors in which a control bit implies a narrow data case.

`soft` separates preference from requirement. Hard constraints must be satisfied or the constraint solver fails; a soft constraint is discarded when no solution satisfies it alongside the active hard ones. Its purpose is layering: the author of a generic component supplies defaults that a later, more specialized constraint overrides automatically, where a hard default would force the test to disable that default procedurally or to extend the class just to put another default in its place [cit:S8]. In the DMA
...
Unsatisfiability is the explicit failure. When the constraints admit no solution, not a single input is allowed at the current state, so that state is a dead-end and simulation has to be aborted; over-constraints stall the progress of simulation and, in formal verification, they lead to vacuous proofs, which succeed only because nothing ever satisfied the property's antecedent, so nothing was checked; tools provide constraint diagnosis for that reason, to identify a minimal set of constraints responsible for the conflict [cit:B6]. The antecedent of a property is the condition that must become true before the property checks anything. A failed `randomize()` exposes an unsatisfiable set of constraints within seconds. Checking the return value of `randomize()` can be necessary for exactly that reason, a set of constraints with no solution [cit:S8]; treating the check as mandatory is the practitioner's rule on top, because an unchecked failure leaves the object holding its previous values and the run continues, re-sending the last legal descriptor forever.
...
**Worked example: exclusion of the canonical bug.** In week 4 of the DMA campaign, a constraint is added after a failure review, and it excludes the canonical bug. The canonical bug is the recurring DMA defect that this book follows from plan to fix. An engineer sees descriptors with rows straddling a 4 KB boundary and concludes they are illegal because no AXI burst may cross a 4 KB boundary (Arm IHI 0022H.c §A3.4.1 "Address structure") [cit:S18], adding:
...
The constraint is *true of the design's output* and *false of its input*. The 4 KB rule governs the bursts that the backend may emit (Arm IHI 0022H.c §A3.4.1 states the rule of a burst, and because that rule speaks of bursts rather than of the descriptors the stimulus programs, the constraint is wrong of the *input*) [cit:S18]; splitting a request that would cross a 4 KB boundary is the burst splitter's job, the feature that the campaign exists to verify. Imposing the DUT's obligation on stimulus leaves tests passing because they no longer exercise its faulty behavior.

The canonical coverage model `cg_2d_4kb` (Chapter 5) crosses four axes: stride sign, destination-row relation to the next 4 KB boundary, burst length and other-channel activity, giving 3 × 4 × 3 × 2 = 72 cells. The new constraint makes the `straddle` bin of `cp_boundary_dist`
unreachable, and with it every cell containing it: 3 × 1 × 3 × 2 = 18. Not all eighteen were reachable; deriving the difference supplies the evidence needed by a waiver, the written record that accepts one hole in the evidence and names its reason, its risk and its owner.

A coverpoint is an expression that a coverage model partitions into bins, the counters that record which values a run actually produced. The two axes sample at different scopes: `cp_boundary_dist` classifies a destination *row*, while `cp_len` samples the length of a burst as the burst splitter actually emits it, after any split. `c_solver_budget` caps a 2D row at 2048 bytes (256 beats on the 64-bit path, one maximum burst, half a 4 KB region), so a row reaches past at most one 4 KB boundary. A straddling row splits into exactly two bursts, and the beat counts of those two bursts sum to the beat count of the row, at most 256, with each burst carrying at least one beat.
Neither piece can be 256. `straddle` × `max` is therefore empty for every stride sign and both channel states: **6 of the 18 were unreachable before anyone wrote the week-4 line**. Subtracting those six from the eighteen leaves twelve, and what `c_no_page_cross` costs is the other **12**. The
canonical DMA bug (a negative-stride row mis-split at the 4 KB boundary, corrupting data only when channel 1 is also active) sits in `neg` × `straddle` × other-channel-active, one cell per burst-length bin; the `max` one sits in the excluded column, so the constraint deletes **both** of the bug's live cells.

A plan row is the line of the verification plan that names one feature, the method that tests it and the evidence that closes it; plan row DMA-F06a needs two caveats. The six exclusions depend on *policy*: without the solver budget, a 4104-byte row at offset 4088 splits into 1 beat and 512, with the 512 splitting again into 256 + 256, making `straddle` × `max` reachable. And the whole argument is
destination-side: `c_no_page_cross`, `c_addressable` and `dst_row[]` all speak
about writes. Chapter 5's *100% means 100% of the cells the model can legitimately reach* requires this bookkeeping, separating what is impossible from what was merely decided; an unrecorded exclusion can lead a sign-off to quote 72, the model's full cell count. A sign-off is the review that judges the evidence against the plan.
...
3. **Targeting the missing cell.** Request the relation defining the missing *cell* so a randomization failure can diagnose the constraint conflict.
   ...
   ```
   The second clause carries the diagnosis. It is satisfiable in `dma_2d_descriptor`: offset 4088 plus a 2048-byte row reaches 6136. It is unsatisfiable in `dma_desc_overconstrained`: `c_rows` places row 0 at `dst_addr`, and `c_no_page_cross` then requires that row to end inside one 4 KB region, which is the negation of the relation the call requests. Targeting only `dst_stride` instead permits an 8-byte row, which fits inside one 4 KB region at every legal offset and therefore cannot expose this conflict.
...
`c_frontier` constrains the *expression classified by the coverage model* rather than a raw address. The third range maps onto `straddle`. The first two land somewhere among the other three bins without choosing which, since those bins are classified against a burst's worth of headroom rather than against the boundary. The expression names row 0, while `cp_boundary_dist` samples every row. The `c_frontier` constraint therefore targets one bin of four, on the first of up to sixty-four rows; it does not target all four. In this example, software rarely uses the signed stride field, so `c_stride_sign` gives legal negative strides more mass than positive ones to increase their occurrence.

`c_len_extremes` and `c_frontier` both constrain expressions containing `num_bytes`, and `c_order` orders `num_bytes` itself against `dst_addr`. The constructs are legal: no `randc` variable is involved, and each distributed expression contains a random variable. A `randc` variable is a random variable that cycles through every value of its range before any value repeats. Weights hold *absent any other constraints*; where
...
Generic constraint solving is NP-hard; SAT is NP-complete, and its solutions are *believed* to have an exponential worst case; SAT performance varies widely across similar-looking instances, and SAT is one approach among several: linear and integer programming, automatic test pattern generation, binary decision diagrams, Boolean unification and constraint logic programming [cit:B6]. The tool chooses its solving engine; the problem does not prescribe one. In practice, small randomized objects and bit-selects instead of arithmetic reduce cost; a generator suddenly taking 40 seconds per transaction warrants a bug report.

Manual shaping adds overhead, motivating Chapter 6's automation research. Using distribution and ordering directives well demands understanding both their semantics and the solving process, for three reasons. Distribution constraints carry order semantics, and misunderstanding those semantics yields a distribution different from the one intended. In a large model the relations between fields are tangled, so a directive on one field can change the distribution of others. And the engineer must know the whole coverage model, since not all objectives can be met at once.
...
Reproduction infrastructure must make it *easy* to identify which versions of which sources, tools and libraries were used and to reissue the exact command, especially the seed. Command-line options cannot be source-controlled, so a script should set the detailed options from broad, high-level ones. A testbench reused across simulator and constraint-solver versions may generate different stimulus for the same seed [cit:B1].

**Random stability** keeps seeds useful while the testbench changes. The RNG is localized to threads and objects, so the values one object or thread produces are independent of the others. Each class instance gets its own RNG, seeded from the thread that creates that instance. Each new dynamic thread also gets its own RNG, seeded from its parent thread. Under that hierarchical seeding a whole subsystem can be pinned by manually seeding one root thread. Stability is
...
**Seed count and coverage.** Low input coverage after only a few runs calls for further sampling (in the source's example, 8 of 10 opcodes after three runs); the same result after many runs (twenty-five, in that same example) calls for reviewing the generator's distribution.
...
inputs that point needs [cit:B2]. A test that misses points it was *designed* to hit needs repair. Points it was never designed to hit belong in a new one, so the
existing test keeps working and keeps the coverage it earns [cit:B1].

**Run length.** One long run and many short ones produce equal-quality random data if the source is fully random and the seeds do not repeat a previous sequence, but their project effect differs: a long run is cumbersome to reproduce when the error appears near the end; it exercises a single device configuration; and it can leave the DUT in one region of the state space, where further stimulus adds nothing; many short runs, by contrast, reproduce faster, sweep more configurations and traverse the space more quickly [cit:B2]. The state space is the set of configurations the design's state elements can take. Many short runs are therefore preferable; Chapter 12 develops their regression economy.
...
activity [cit:R2]. A directed test that fails names the intent it was written to check, whereas a random run that fails supplies a log and a seed. Two inexpensive practices help: have every generated item print a human-readable description of itself, and record the seed in the failure report so triage begins with a command that reproduces the failure. Triage is the sorting of failures into facts before any one of them is debugged.
...
**Atomic.** Individually constrained transactions suit cases needing no constraints across a *sequence*, such as a configuration descriptor [cit:B1]; for the DMA, the atomic item is one descriptor.

**Scenario.** This layer builds sequences of random transactions with defined relationships, each sequence aimed at a particular functional corner. The scenario is described and randomized as an *array* of items solved all at once, letting the constraint solver relate past, current and future items in one constraint set [cit:B1]. Randomizing three descriptors in turn cannot express "the third descriptor reuses the second's destination".

> **Scenario.** A USB controller's link power states (*Universal Serial Bus 3.2 Specification*, Revision 1.1, USB 3.0 Promoter Group, June 2022, section 7.5, "Link Training and Status State Machine (LTSSM)") [cit:S21]. **Approach.** The atomic item is a packet; the scenario traverses U0 to U1, an inactivity timeout to U2, and a remote wake back to U0, with transfers scheduled *relative to* those transitions. U0 is the link's active state; U1 and U2 are low-power states the link can rest in, and U2 is the deeper of the two. **Why.** Scheduling a transfer in the cycle of low-power entry requires a relationship between two items and a timer, beyond the constraints on either item.

**Multi-stream (virtual).** The third level addresses one technical limitation: constraints cannot be expressed *across* scenarios in different streams, because those items are generated in different generators and are not inside the object that any one generator randomizes. Expressing cross-stream relations requires that the descriptors for all streams be randomized as a single object. Three arrangements are alternatives: synchronize otherwise independent generators; let a single-stream
...
**Worked example: one object, three streams.** Two canonical bugs require conjunctions that a single-stream generator cannot request. The neural accelerator's weight prefetcher wraps its buffer pointer one entry early when the weight stream ends mid-beat *under DMA contention*, reachable only at 2-bit weights with a width-1 tensor. A tensor's width is the number of elements in one row of the operand that the accelerator convolves. The DMA's mis-split at the 4 KB boundary corrupts data only when channel 1 is also active, the axis Section 9.3 could not reach within one descriptor. Both are relations
...
For an Ethernet MAC with TSN (time-sensitive networking) queues, the atomic item is a frame and the scenario a traffic profile (a burst class at a stated line occupancy). A multi-stream descriptor correlates a high-priority queue profile with a best-effort flood on another queue *in time*, which is required to provoke a shaper-arbitration corner. The shaper is what paces the frames of each queue, and arbitration is what picks the next queue to send when several are ready at once. Layers must be interruptible: the scenario layer
...
Everything so far assumes a testbench that owns the interfaces. AMD describes the limits of this assumption at SoC level: at IP level, the constrained-random testbench is excellent, but the initialization sequences it contains are not easily portable to firmware or post-silicon tests, and IP-level tests lack system context; at SoC level, the stimulus is simple directed feature tests, complex scenarios are difficult to create by hand, and run times are long; post-silicon, tests are OS-based, debug takes longer, and failures are difficult to bring back to simulation or to emulation [cit:D29]. In emulation a design is compiled onto a hardware fabric and runs under stimulus a host supplies, before silicon exists.
That source is a conference slide deck: it describes AMD's experience and the flow built in response, rather than survey findings.
...
randomizing *between* sequences is difficult. A constraint class can express one resource-contention configuration with display-controller streams competing for pipes, overlays and interfaces, but that captures one specific scenario, not the ability to start and stop streams while others are in progress or to schedule them arbitrarily; it does not capture the underlying problem, which is tasks contending for limited resources [cit:D5]. A pipe is one of the display controller's paths for fetching a layer of pixels, and an overlay is one of the layers the controller composites into the output.
...
instantiation. Behavior is an **action**. An action is atomic when it maps directly to a system operation, and compound when it encapsulates a flow of actions in an **activity**. Actions consume and produce passive **objects** whose type fixes
...
In PSS a **scenario** is one particular solution of all those constraints [cit:S1], and not the sequence of stimulus the word names in Section 9.5.

What distinguishes PSS from a constraint class is **inference**. Where the specification of intent is partial, the tool shall infer the additional actions and model elements needed to make that specification complete and valid, and shall infer nothing beyond what is required, so one partial specification expands into a variety of scenarios that all satisfy the stated intent while including other behaviors that broaden coverage (PSS 3.0 §5 and §5.3.2) [cit:S1]. For the intent "do a 2D transfer, then
...
dependency between them may or may not overlap in time (PSS 3.0 §6.3.1, §6.2.3) [cit:S1]. `schedule` allows any legal order, including entirely sequential branches, so the action named *contention* may run without its branches ever overlapping.
...
called by no activity at all, get their place in the generated test. The same `contention` action becomes transaction sequences that drive a bus verification IP in a block-level simulation, and a bare-metal C program running on the core in emulation or on silicon. A bus verification IP is a component supplied ready-made for a standard protocol.

PSS-described intent was compiled into UEFI firmware tests and bare-metal tests, and those tests ran unchanged on an AMD accelerated processing unit, first under emulation and then on post-silicon parts; they reached the hardware through the Cadence Perspec portable-stimulus tool and through an in-house AMD runtime layer called uDREX, whose job was to abstract the BIOS and provide services to the generated tests; in the lab the same flow was then used to generate fresh power-management firmware tuning scenarios [cit:D29]. The account reports retargeting and its application without quantifying speed-up, bug count or effort savings.
...
Two more come from the standard rather than from that deck. The value concentrates in scenario composition rather than in checking: expected-result computation, memory modeling and allocation policies may each need outside help, and that help comes as external models, software libraries or algorithmic code in other languages (PSS 3.0 §1.4) [cit:S1]. The standard permits them; it does not supply them, and specifying
results checking inside the model remains a stated goal rather than a delivered
construct. Test generation leaves the oracle, the mechanism that decides whether an observed response is correct, to the verification environment (Chapter 3).
...
**Exercise 9.1 (calculation).** Take the implication over a one-bit and a thirty-two-bit random variable that §9.3 uses to introduce `solve ... before`. Derive the size of its legal set and the probability that the wide variable takes the value the implication forces, under the uniformity the constraint solver guarantees. Then derive the same probability once the ordering directive is added, and account for the word `slightly` in the figure §9.3 reports for it.

**Exercise 9.2 (calculation).** Take the row length and the destination offset that §9.3 uses in its first caveat to plan row DMA-F06a, namely a 4104-byte row at offset 4088. Derive the two pieces the burst splitter produces and the further split of the larger piece, giving the beat counts on the DMA's data path. State which premise of the six-cell exclusion the example lifts, and state what §9.3 then requires be recorded on the plan row.
