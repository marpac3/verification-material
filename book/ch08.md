...
The DMA team also needs to reuse the AXI checking against its backend. Reuse fails because the checker reads driver-written variables: it knows only the intended stimulus, never what appeared on the wires, so it cannot provide an independent oracle, the mechanism that decides whether an observed response is correct (Chapter 3).
...
{table: five_responsibilities} The five responsibilities a testbench separates, each with the question it answers and the thing it must not know. The third column carries the argument: every entry in it is an ignorance imposed on purpose, and merging two of these boxes does not merely blur the environment's structure, it gives one box knowledge that the table forbids, for instance a checker that knows which test is running.
...
Functional verification is the establishment, before fabrication, that a design implements its specification. By 2001 the same principle was already the basis of a language designed for functional verification: that language had a data model, a test generator, a reference model, a checker and coverage metrics tied back to a plan; its checker was split further into a data checker and a temporal checker running concurrently with the design; and its organizing principle was separation of concerns rather than a coding convention retrofitted later [cit:P18]. Chapter 9 traces this history: the structure preceded the tooling.
...
comparison move together. Checking inside the driver therefore embeds Chapter 3's common-mode error in the architecture, the same specification misreading encoded in both the design and its oracle.
...
On a finished project, the self-checking structure is the largest and most complex part of the environment. It is the part that cost the most to author and maintain; it is the part responsible for declaring functional correctness; and it embodies a duplication of the specified functionality of the design. Checking needs planning before bring-up. The verification plan must state which failure modes the checking detects and how it detects them, and that statement must be reviewed, so that a potential failure does not go undetected. The classical categories are data transformation, data ordering, protocol correctness, data losses and design state [cit:B2] (see Chapter 5).
...
At the **signal level** the environment works with pins, edges and handshakes. At the **transaction
...
than the design's clock cycles [cit:P18]. At the **sequence level** the environment composes scenarios: ordered and constrained collections of transactions that express intent ("fill the write-data channel of manager 2 while the debug port reads the ROM"). Chapter 9
...
**Operational** communication is point-to-point and blocking: a producer hands a transaction to one consumer, and back-pressure is meaningful. The sequencer is the component that holds the sequence requests waiting to run and chooses which one the driver receives each time the driver asks, so a sequencer that cannot hand its next item to a busy driver *should* wait. **Observational** communication broadcasts a monitor's observations without blocking or depending on listeners. Its standard form is an analysis port with a single `write()` function broadcasting a value to all subscribers [cit:S9]. The port may have zero, one, or many subscribers without affecting the producer's operation; however many are connected, the void `write()` call always completes in the same delta cycle, that is, without advancing simulation time [cit:S11].
...
Broadcast requires independent copies: each subscriber receives a handle to the *same* transaction object, so every subscriber must copy before operating on it [cit:S11]; the standard states normatively that a subscriber's `write()` implementation shall not modify the value passed to it (IEEE Std 1800.2-2020, *IEEE Standard for Universal Verification Methodology*, §12.2.10.2) [cit:S9]. A coverage collector that normalizes an address in place silently corrupts the scoreboard's expectation, and the result looks like a design bug.
...
Traditionally a transactor is tied to one physical interface, and such a transactor prevents reuse: each environment needs its own model and its own abstraction level. Layered models follow the protocol's structure. The layers are usually obvious in communication
...
What counts as a transaction at each layer stays a local decision. TLM is the passing of whole transactions between components over standard interface handles rather than through signals. TLM-2.0, part of IEEE 1666 and built on SystemC, supplies its interoperability layer [cit:S25], and in practice that interoperability has held only for memory-mapped bus protocols: for serial and interrupt-style interfaces such as SPI, I2C, CAN and UART, each modeling team defines a payload of its own [cit:D43]. SystemC is a modeling language whose models sit above the level of signals.
...
A third benefit is retargeting. An emulator is a purpose-built machine with a compiler and transactors of its own, big enough to hold a whole design and observable through triggers and state saving. Compact transaction messages replace per-signal traffic to overcome the throughput limits of a simulator or an emulator, and they do so under two conditions: timed and untimed behavior stay separate, and the two sides synchronize on transactions and events rather than clocks [cit:D33].
...
A scoreboard is a data-structure problem decided by one operation, lookup: because the exact order in which outputs appear may be hard to predict, the structure must be designed to minimize the look-up operation [cit:B1]. The following three patterns differ in their ordering assumptions.
...
A transfer descriptor is the record that programs one DMA transfer. The skeleton of that in-order pattern runs on the completions of one DMA channel's transfer descriptors: the predictor fills the queue, the observation consults its front, and Section 8.6 turns the single queue into one per stream key.
...
> **Scenario.** The Ethernet MAC's time-sensitive networking (TSN) queues reorder by design: eight
> priority queues with a credit-based shaper mean a low-priority frame injected first
> can legitimately leave last. **Approach.** Key expectations by (destination port,
...
executable to compare against. Checking combines weaker claims: the output decodes, it decodes to a picture within the distortion target *the team itself* set (a quality goal of its own making, not a standard-mandated one), and it never pushes the level's buffer-occupancy constraints out of bounds. Each of those claims is checkable, and none of them is golden. One block thus has a bit-exact model in one direction and no model at all in the other, so golden status does not establish completeness.
...
[cit:P25]. Without notification, a random interrupt can create an environment-induced divergence; in this example, diagnosis takes a day. The same rule governs any responder whose behavior the model must know about, and error injection, the deliberate driving of erroneous stimulus to check that the design detects the error and recovers from it.

The OpenHW Group's CV32E40Pv2 is a 32-bit in-order RISC-V core, and its environment runs the same program on the RTL and on the reference model, streams the core's internal state and its asynchronous events over an interface named RVVI-TRACE, and compares state as each instruction retires, flagging a mismatch the moment it appears rather than at the end of the run; the core originated as RI5CY at ETH Zurich, Dolphin Design continued its development, and Imperas supplied the reference model and the coverage model [cit:D6].
...
The plan must budget this maintenance (Chapter 5) before it emerges in month four. Without a model, expectations are hard-coded per test; hard-coding needs a known configuration and a known input stream, so it works only for directed testcases; and because the response checked is crafted for that testcase, an unexpected corner that some other test creates by accident goes undetected [cit:B2].
...
A late response task leaves the testbench unready: internal back-pressure prevents full-throughput verification, output data "spills" and leaves stream gaps, or missing feedback signals cause an output protocol violation attributable entirely to the testbench. The structural answer is the **autonomous monitor**: an independent thread started at construction that continuously watches the interface and pushes extracted data into a FIFO that the rest of the environment drains independently; the arrangement decouples the timing of the transaction from the timing of the checking [cit:B2].
...
A virtual interface handle is the reference through which a class component reaches an interface's signals. A component holding a plain `virtual axi_mon_if` handle reaches every member of the interface, and `awvalid`, `awready`, `awaddr` and `awid` are ordinary `logic` variables: `vif.awvalid = 1'b1;` compiles and drives. A clocking block on its own
...
stimulus half is switched off and the observation half keeps working. Without that separation, the environment is rebuilt at every integration level; and if that rebuilding is omitted, system verification runs with no checking at all.
...
A monitor that infers behavior it cannot observe stops being a monitor and becomes a second implementation of the design.
...
A wire UVC is a verification component, a reusable testbench-side model of one physical interface with a driver to stimulate and a monitor to observe. Verilog-AMS is the analog and mixed-signal extension of the hardware description language. A mixed-signal block taken from a current LSI Corporation design is an analog driver with filtering and fault detection, and its environment was built as wire UVCs: their driver and monitor functions are written not in the testbench language but in Verilog-AMS; and the components are wrapped in the same self-checking sequences, functional coverage and regression control as a digital agent would be [cit:D19].

> **Scenario.** In the flash controller's low-density parity-check (LDPC) ECC pipeline, a first environment makes its NAND-side monitor recompute the ECC codeword to report "correctable" or "uncorrectable" per page. **Approach.** A predictor across the analysis port performs the recomputation; the monitor reports the raw codeword, the page's status bits and the design-published syndrome, the check value a decoder derives from a codeword. **Why.** This addresses two problems: debugging cannot distinguish faulty monitor arithmetic from a design bug when the error originates in an unreviewed component; and reusing the design team's LDPC routine under schedule pressure creates a common-mode check that proves nothing (Chapter 3).
...
**Step 2: monitors publish to subscribers.** Each monitor broadcasts the transactions it reconstructs to three kinds of subscriber. The first is the per-interface protocol checker. The second is the coverage collector, which samples the shaped route cross `cg_xbar_route` of Chapter 6; the model, its `ignore_bins` and their justifications belong to that chapter, and this environment only supplies the stream. An `ignore_bins` declaration excludes a bin from a coverage model's score because covering it is not that model's job. The third is the checking path, which takes two connections on the manager side (requests to the predictor, responses to the scoreboard) and one on the subordinate side (the forward-path comparison). None of those three subscribers can block the monitor, and none may modify the transaction it receives [cit:S9]. {ref: crossbar_env} shows the resulting structure.
...
expected beats. The predictor is not golden: if its map and the RTL parameterization disagree, every region-boundary access produces a mismatch, and the first response is a specification review rather than checker debugging (Chapter 3).
...
The two-coordinate key (subordinate, ID) fails all three requirements. Manager 1 and manager 2 may both have ID 3 outstanding to subordinate 0, and Chapter 10 deliberately sets that situation up with a virtual sequence, a sequence that coordinates several interfaces at once. One queue then matches one manager's expectation against the other's response. Only some arbitrations produce that pairing, so the fault surfaces as a mismatch that disappears on a seed change rather than as a reproducible bug. The two-coordinate key also merges `ARID` and `AWID`, which name unrelated streams, so a read expectation can be popped by a write response. An interconnect merging several managers must also *widen* subordinate-side IDs to avoid falsely serializing different managers' same-ID transactions. Five managers need ⌈log₂5⌉ = 3 manager-identifying bits. Those three bits are added to the 4-bit ID a manager issues, so the same transaction arrives at a subordinate port with a 7-bit ID. Keying the scoreboard there is sound only with that widened ID, and it obliges the predictor to reproduce the design's ID-remap policy exactly. Manager-side keying avoids modeling that policy and keeps the key computable from what the observer sees.
...
zero-delay functions, so a checker can never back-pressure a monitor [cit:B1]. The comparison uses `!==`, not `!=`, because logical inequality on operands containing x or z yields `1'bx`, and an `if` statement reads that value as false: with `!=`, a single undriven beat scores as *matched*; the comparison could not detect the undriven beat. The `$isunknown` guards prevent `int'(id)`, a two-state cast, from collapsing an unknown ID to `0` and filing a corrupt transaction under a real key. A response that never arrives produces no mismatch, only a queue that never empties, so `leftover()` checks for this usually silent failure [cit:B2]. `report()` exposes these counters so their results can be inspected.

**Step 5: limits of the environment.** The canonical crossbar bug is write-data interleaving under back-pressure: beats belonging to two writes in flight to one subordinate appear interleaved on the wire, on a channel that AXI4 obliges the crossbar to keep in address order (Arm IHI 0022H.c §A5.2.2 "Write data ordering") [cit:S18]; interleaving write data with different IDs is deprecated in AXI4 and later rather than forbidden, and address order is one burst at a time because there is no `WID`, the signal that would tag each write beat with the ID of its burst. The scoreboard cannot detect this because a tolerant subordinate still stores correct bytes and returns correct responses; the violation relates two transactions rather than either transaction's data. It was
found by protocol assertions in formal, with an 11-cycle counterexample (Chapter
14). A counterexample is the input sequence a proof engine returns to exhibit a failure. The scoreboard checks return-path data and completeness, the forward-path check owns routing, and the protocol monitor owns legality. Formal provides exhaustiveness for the legality properties that matter most; fairness and starvation remain outside all these checks, as Step 0 declares.
...
error/interrupt monitor publishes completion and error events. The oracle is a **descriptor-level golden model**, a C model of the transfer semantics (source, destination, 1D/2D sizes, strides, element width) that is co-simulated through the language's foreign-interface layer and that turns the programmed descriptor into the expected destination memory image.

This model is golden within a narrow scope: data-mover transfer semantics *are* the specification, and the model implements them at byte-movement abstraction rather than burst abstraction. Leaving out the burst splitter's split decisions is what makes the model maintainable. Section 8.4's Species 1 model of the burst splitter predicts those split decisions and is a transfer function as buggy as the design it mirrors; the descriptor model predicts only byte destinations, and it stays golden because it excludes splitting. The two models for one IP differ in the abstraction level each commits to.

Both checking levels are necessary:

**Level 1, end-to-end and late.** At descriptor completion, checking compares the memory-model image with the golden image byte for byte, and it compares the reported per-channel status with the predicted status. That comparison catches the canonical DMA bug: the burst splitter mis-splits a negative-stride 2D transfer at the 4 KB boundary, and the mis-split corrupts data only while DMA channel 1 is also active. The malformed burst is masked unless channel 1 owns the shared AXI backend in the same window, because arbitration timing hides it otherwise. The stride is the signed address step from the start of one row to the start of the next. Every burst the
backend issued was individually legal; only the bytes were wrong.

**Level 2, per-burst and immediate.** Protocol assertions are bound to the backend's AXI channels, among them the 4 KB-crossing property established in Chapter 2 and re-examined as a partial oracle in Chapter 3. Beside them sits the coverage model `cg_2d_4kb`, which records how the burst splitter treated each 2D row, the contiguous run of bytes that makes up one line of a two-dimensional transfer. Chapter 5's plan row DMA-F06a names that model, and Chapter 6 treats it. The per-burst assertions fire on the offending beat.

Both levels are necessary because detection distance has a cost: errors should be detected as early as possible, while the model is still near the failing state [cit:B2]. An end-to-end mismatch reports 4 KB of incorrect destination memory at completion, and on this block it arrives an order of magnitude late, tens of microseconds and hundreds of bursts after the mistake; the assertion identifies the offending beat. The checks are complementary: assertions cannot see wrong data, and image comparison cannot identify *when* the error occurred.

Two further decisions prevent false failures from over-specification. **Exact split sequences over-specify the model:** checking them turns every legitimate change to the burst splitter's arbitration into a regression failure. Checking instead requires bursts to stay inside one 4 KB region each, to cover the descriptor exactly once in aggregate and to match byte counts; coverage records which split shapes appeared. **Asynchronous events reach both sides:** an error response injected by the responder on beat 7 must also reach the golden model, or it predicts a complete image while the design correctly reports a partial transfer. This applies Section 8.4's processor co-simulation rule to the data mover [cit:P25]. Letting the golden model inspect the burst stream during bring-up diagnostics turns it into a second implementation of the burst splitter, with its own bugs and no independence (Chapter 3).

The burden belongs in the plan: a change to the way transfers are split touches the RTL, the model and the coverage model, and the review must cover all three. The flagship's DMA, with eight channels, 40-bit physical addressing and descriptor chaining, reuses this environment: the golden model adds an address-width parameter and chaining loop, the scoreboard key adds a DMA-channel dimension, and the 4 KB discipline stays unchanged. The canonical DMA bug's flagship descendant needs exactly one new component, a coherence-aware memory model, because the second half of its split writes a line that the host cluster holds in the Modified state, the state in which one cache holds the only up-to-date copy of a line.
...
**Reuse contract.** The self-checking structure belongs in a class with three properties: the class holds no reference to the verification environment around it; it exposes a procedural interface through which stimulus, exceptions and observed responses are reported; and it works with zero delay rather than as a process of its own, forking a blocking thread out of that interface on the rare occasion it needs simulation time; that zero-delay rule is also why inter-process channels and mailboxes are the wrong interface to a checker [cit:B1]. A checker importing the block-level environment in week three prevents its own system-level instantiation and the reuse planned at kick-off. With that independence, one environment per IP composes into cluster environments and then the top-level environment [cit:S11].
A verification consultancy rebuilt its scoreboard as a configurable library, so that the design-specific part became configuration and not new code, and it reports two results: its projects typically obtain an approximate 15% reduction in scoreboard code against starting from the empty base class, and they typically complete setup and configuration inside a day even for complex designs; the library is published under a permissive open-source license [cit:D41].
...
**Bring-up debuggability.** Four properties support bring-up debuggability: every transaction carries an identifier and a timestamp; every failure message names expected, observed *and* the provenance of the expectation; the environment has a mode with checking disabled, to separate "stimulus cannot reach this" from "the check is wrong"; and the checkers are themselves tested by injecting a known-bad response and confirming the environment fails. Checker credibility requires evidence that it can fail (Chapters 2 and 3). The claim that these four properties repay their own cost within a week of bring-up is an illustrative assumption. All four need budgeting: this is the environment's largest part, with the greatest complexity and the most maintenance [cit:B2]. It requires the same review, coding standard, lint and regression discipline as RTL, with its schedule in the plan (Chapters 4 and 5).
...
- A scoreboard is a data structure whose shape is decided by the lookup [cit:B1,B2].
...
- The environment is a design: a checker's reusability requires independence from its surroundings and a procedural interface [cit:B1]. The self-checking structure is the largest and most maintained part of the environment [cit:B2], so its plan must provide for review and testing.

### Exercises

**Exercise 8.1 (calculation).** Take the crossbar parameters listed at the head of §8.6 together with the stream key of Step 4. Compute how many in-order expectation queues that key can address, and how many the two-coordinate key that the same step rejects would address. Step 4 holds that the shorter key fails all three of the concurrency sources it lists: give the reason it fails each one, and name the way Step 4 says that failure shows itself when two managers hold the same identifier at once.
...
**Exercise 8.4 (artifact).** Read the end-of-test line below, printed by the `report()` function of the crossbar scoreboard in §8.6. State the value `report()` returns for these counts and the reason. Name the design behavior that a nonzero final field records and the thing it leaves open, and name the pitfall of the chapter that an environment without that field matches.
...
**Exercise 8.6 (judgement).** A reviewer proposes replacing the descriptor-level golden model of §8.7 with the burst-splitter model of §8.4's Species 1, so that one oracle predicts burst splitting as well as destination bytes. Decide what the substitution costs, naming the scope in which the descriptor model is golden and the reason the burst-splitter model cannot inherit that status. State the over-specification §8.7 warns against and its effect on the regression.
